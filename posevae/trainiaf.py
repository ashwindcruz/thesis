...
vae = modeliaf.VAE(d, nhidden, nlatent, zcount)
#opt = optimizers.Adam()
...
with cupy.cuda.Device(gpu_id):
    while True:
        ...
        if now >= print_at:
            ...
            Elogp = logp_mean / obj_count
            Elogq = logq_mean / obj_count
            print "   %.1fs of %.1fs  [%d] batch %d, E[obj] %.4f,  E[log p] %.4f, E[log q] %.4f, %.2f S/s, %d total" % \
                  (tpassed, runtime, printcount, bi, EO, Elogp, Elogq, tput, total)
        ...
        obj_mean += obj.data
        obj_count += 1
        logp_mean += vae.logp.data
        logq_mean += vae.logq.data
